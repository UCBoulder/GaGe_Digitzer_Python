...
def stream(
    ...
    Args:
        ...
        args (None, optional): any additional arguments to pass for real-time analysis
    """
    # %% ====== handle and config =============================================
    (
        handle,
    ...
    # variables for DoAnalysis
    (avg_segment_size,) = args
...
def DoAnalysis(mp_values, mp_arrays, workbuffer, g_cardTotalData, avg_segment_size):
    mp_g_cardTotalData, mp_average = mp_values, mp_arrays

    # unpack from buffer
    x = np.frombuffer(workbuffer, "<h")

    # resize data
    N = int(np.round(x.size / avg_segment_size))
    x.resize((N, avg_segment_size))

    # sum the data
    mp_average[:] = np.sum(x, axis=0)

    # update total data size
    mp_g_cardTotalData.value = g_cardTotalData[0]


if __name__ == "__main__":
    ppifg = 77760  # average segment size
    buffersize = int(77760 * 1000 * 2)
